...
% load("v11-sin-passive-slow-data.mat")
load("v11b-test.mat")

% v = VideoWriter('v11-sin-passive-slow.mp4','MPEG-4');
v = VideoWriter('v11b-test.mp4','MPEG-4');
v.Quality = 99;
v.FrameRate = FPS;
open(v);
...
%% Plot the center of wheel A.

j_wheelA = zeros(2,numel(t_anim));

for iter = 1:numel(t_anim)
    j_coords = rj_func(q_anim(iter,:).');
    j_wheelA(:,iter) = j_coords(:,4);
end

plot_j_wheelA = plot(j_wheelA(1,:), j_wheelA(2,:), 'b-','linewidth',1) ;

plot_j_wheelA.HandleVisibility = "off" ;
